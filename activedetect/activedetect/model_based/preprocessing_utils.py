...
def tryParse(X):
	vals = []
	if (len(X) == 1 and len(X[0]) == 1) or X.shape == (1,1):
		try:
...
def featurize(features_dataset, types):
	...
	for i,t in enumerate(types):
		col = np.array([f[i] for f in features_dataset])
		#print("Type", t)
...
def get_acc_scores(ytrue, ypred, yscores=None):
	...
	return [accuracy_score(ytrue, ypred), f1_score(ytrue, ypred), roc_auc_score(ytrue, yscores, average='weighted')]
