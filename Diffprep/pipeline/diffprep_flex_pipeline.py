...
class Transformer(nn.Module):
    ...
    def init_parameters(self, beta):
        ...
        print("Setting transformer is sampled", self.is_sampled)
    ...
    def numerical_diff(self, X, alpha, eps=1e-6):
        ...
        X_sample_grad = (X_grads * self.tf_prob_sample.detach().numpy()).sum(
            axis=2
        ) * alpha.detach().numpy()
    ...
    def forward(self, X, is_fit, X_type, max_only=False, require_grad=True, alpha=None):
        ...
        for tf in self.tf_options:
            if is_fit:
                X_t = tf.fit_transform(
                    X.detach().numpy()
                )  # transformer sample change, X_output change
            else:
        ...
        X_output = self.select_X_sample(
            X, X_trans, max_only, require_grad=require_grad, alpha=alpha
        )
        return X_output
    ...
    def categorical_sample(self, logits, temperature, use_sample=True):
        ...
        if not use_sample:
            ...
            # print(self.name)
            # print(logits)
            samples = torch.distributions.RelaxedOneHotCategorical(
                temperature, logits=logits
            ).rsample()
            indicator = torch.max(samples, dim=-1, keepdim=True)[
                1
            ]  # find one-hot position, an int
            one_h = torch.zeros_like(samples).scatter_(
                -1, indicator, 1.0
            )  # change [0,...,0] to one-hot
    ...
    def sample(self, temperature, use_sample=True):
        self.tf_prob_sample = self.categorical_sample(
            self.tf_prob_logits, temperature, use_sample
        )
...
class FirstTransformer(Transformer):
    ...
    def fit_transform(self, X):
        ...
        if self.contain_num:
            ...
            X_num_trans = torch.Tensor(np.array(X_num_trans)).permute(
                1, 2, 0
            )  # shape (num_examples, num_features, num_tfs)
        ...
        if self.contain_cat:
            ...
            X_cat_trans = torch.Tensor(np.array(X_cat_trans_enc)).permute(
                1, 2, 0
            )  # shape (num_examples, num_features, num_tfs)
    ...
    def get_feature_names(self):
        ...
        if self.contain_cat:
            cat_feature_names = [
                c for c in self.one_hot_encoder.get_feature_names_out(self.cat_columns)
            ]
        else:
            cat_feature_names = []
    ...
    def init_parameters(self):
        ...
        if self.contain_num:
            if self.init_num_tf_option is None:
                num_tf_prob_logits = torch.randn(
                    self.out_num_features, self.num_num_tf_options
                )
            else:
                init_num_tf_idx = self.num_tf_methods.index(
                    self.init_num_tf_option.method
                )
                init_num_tf_probs = torch.ones(
                    self.out_num_features, self.num_num_tf_options
            ...
            self.num_tf_prob_logits = nn.Parameter(
                num_tf_prob_logits, requires_grad=True
            )  # [feature_num, tf_num]

        if self.contain_cat:
            if self.init_cat_tf_option is None:
                cat_tf_prob_logits = torch.randn(
                    self.out_cat_features, self.num_cat_tf_options
                )  # [feature_num, tf_num]
            else:
                init_cat_tf_idx = self.cat_tf_methods.index(
                    self.init_cat_tf_option.method
                )
                init_cat_tf_probs = torch.ones(
                    self.out_cat_features, self.num_cat_tf_options
            ...
            self.cat_tf_prob_logits = nn.Parameter(
                cat_tf_prob_logits, requires_grad=True
            )  # [feature_num, tf_num]
    ...
    def transform(self, X):
        ...
        if self.contain_cat:
            ...
            X_cat_trans = torch.Tensor(np.array(X_cat_trans)).permute(
                1, 2, 0
            )  # shape (num_examples, num_features, num_tfs)
    ...
    def select_X_sample(self, X_num_trans, X_cat_trans, max_only):
        if max_only:
            ...
            num_tf_prob_sample, cat_tf_prob_sample = (
                self.num_tf_prob_sample,
                self.cat_tf_prob_sample,
            )
        ...
        if num_tf_prob_sample is not None:
            X_num_trans_sample = (X_num_trans * num_tf_prob_sample.unsqueeze(0)).sum(
                axis=2
            )

        if cat_tf_prob_sample is not None:
            X_cat_trans_sample = (X_cat_trans * cat_tf_prob_sample.unsqueeze(0)).sum(
                axis=2
            )
...
class DiffPrepFlexPipeline(nn.Module):
    ...
    def forward(
        self, X, is_fit, X_type, resample=False, max_only=False, require_grad=True
    ):
        X_output = deepcopy(X)
        # do first step
        ...
        for i in range(self.n_tf_types - 1):
            ...
            for j in range(self.n_tf_types - 1):
                ...
                if resample or not transformer.is_sampled:
                    # if i == 0:
                    transformer.sample(
                        temperature=self.temperature, use_sample=self.use_sample
                    )

                # forward
                X_output_i += transformer(
                    X_output,
                    is_fit,
                    X_type,
                    max_only=max_only,
                    require_grad=require_grad,
                    alpha=self.alpha_probs[:, i, j],
                )
                cur_idx += 1
    ...
    def get_final_dataset(self, X, X_type):
        X_output = self.forward(
            X, is_fit=False, X_type=X_type, resample=False, max_only=True
        )
        return X_output
