...
class DiffPrepSGD(object):
    def __init__(
        ...
        loss_fn,
        model_optimizer,
        prep_pipeline_optimizer,
        model_scheduler,
        prep_pipeline_scheduler,
        params,
        writer=None,
        train_pipeline=True,
    ):
    ...
    def forward_propagate(
        ...
        y,
        X_type,
        require_transform_grad=False,
        require_model_grad=False,
        max_only=False,
    ):
        """Forward pass"""
    ...
    def fit(
        self, X_train, y_train, X_val, y_val, X_test=None, y_test=None, verbose=True
    ):
        # best_val_acc = 0
        ...
        while e < self.params["num_epochs"]:
            ...
            val_loss, val_acc = self.evaluate(
                X_val, y_val, X_type="val", max_only=False
            )
            test_loss, test_acc = self.evaluate(
                X_test, y_test, X_type="test", max_only=False
            )
            ...
            if verbose:
                pbar.set_postfix(
                    tr_loss=tr_loss, val_loss=val_loss, next_eval_time=epoch_time
                )
                pbar.update(1)
    ...
    def train(self, X_train, y_train, X_val, y_val):
        ...
        # self.update_prep_pipeline(
        #     X_train_batch, y_train_batch, X_val_batch, y_val_batch
        # )
        self.prep_pipeline.fit(X_train_batch)
        ...
        tr_loss = 0
        n_batches = 0
        X_train_iter = make_batch(
            X_train, y_train, self.params["batch_size"], shuffle=True
        )
    ...
    def compute_dval(self, X_train, y_train, X_val, y_val):
        ...
        self.model_optimizer.zero_grad()
        output_val, loss_val = self.forward_propagate(
            X_val,
            y_val,
            X_type="val",
        ...
        # dval / dalpha
        dval_dalpha = [
            param.grad.data.clone() for param in self.prep_pipeline.parameters()
        ]
    ...
    def compute_hessian_product(self, X_train, y_train, dval_dw):
        ...
        eps = (
            0.001
            * _concat(self.model.parameters()).data.detach().norm()
            / _concat(dval_dw).data.detach().norm()
        )
        ...
        hessian_product = [
            (x - y).div_(2 * eps.cpu()) for x, y in zip(grads_p, grads_n)
        ]
        self.model.load_state_dict(model_backup)
    ...
    def train_network(
        self, prep_pipeline, X_train, y_train, X_val, y_val, X_test=None, y_test=None
    ):
