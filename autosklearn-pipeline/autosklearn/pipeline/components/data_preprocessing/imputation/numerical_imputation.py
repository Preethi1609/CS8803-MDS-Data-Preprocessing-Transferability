...
class NumericalImputation(AutoSklearnPreprocessingAlgorithm):
    ...
    def transform(self, X: PIPELINE_DATA_DTYPE) -> PIPELINE_DATA_DTYPE:
        ...
        ans = self.preprocessor.transform(X)
        df_ans = pd.DataFrame(ans)
        print(np.argwhere(np.isnan(ans)))
        print("DFFFF")
        print(df_ans)
        null_counts = df_ans.isnull().sum()
        path = '/home/preethi/projects/CS8803-MDS-Data-Preprocessing-Transferability/autosklearn-pipeline/file.csv'
        df_ans.to_csv(path, index=False)
        print("NULL COUNTS AFTER NEW", null_counts)
        return ans
